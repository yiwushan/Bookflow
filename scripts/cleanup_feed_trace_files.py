...
def collect_expired_trace_files(trace_dir: Path, older_than_days: int, pattern: str) -> list[Path]:
...
def cleanup_trace_files(
    ...
    dry_run: bool,
    delete_only: bool,
    pattern: str = "tr_feed_*.json",
) -> dict[str, object]:
    candidates = collect_expired_trace_files(
        trace_dir=trace_dir,
    ...
    if not dry_run and not delete_only and archive_dir is not None:
        archive_dir.mkdir(parents=True, exist_ok=True)

    for src in candidates:
        if dry_run:
            continue
        if delete_only or archive_dir is None:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Cleanup/archive feed trace JSON files")
    parser.add_argument("--trace-dir", default="logs/feed_trace")
    parser.add_argument("--archive-dir", default="logs/feed_trace_archive")
    parser.add_argument("--pattern", default="tr_feed_*.json")
    ...
    try:
        ...
        if args.markdown_output:
            write_markdown_report(Path(args.markdown_output), build_markdown_report(payload))
